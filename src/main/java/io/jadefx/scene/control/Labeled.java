...
public abstract class Labeled extends Control implements StyleBackground {
	...
	protected void stylePush() {
		...
		Stylesheet.findAndApplyStyle(this.getScene().getContext().getCurrentStyling(), this, this,
				StyleOperationDefinitions.FONT_SIZE, StyleOperationDefinitions.FONT_FAMILY,
	...
	protected void size() {
		...
		if (textWidth > this.getWidth()) {
			...
			for (int i = 0; i < this.text.length(); i++) {
				...
				float tempWid = NanoVG.nvgTextBounds(this.getScene().getContext().getNVG(), 0, 0, tempBytes,
						new float[4]);
			...
			boolean isUpdated = this.textInternal != null && this.text != null
					&& textInternal.capacity() < this.text.length();
	...
	/*
	 * @Override protected double computePrefWidth() { float textSize =
	 * getTextBounds()[2]-getTextBounds()[0]; textSize += this.padding.getWidth();
	 * 
	 * double computedSize = super.computePrefWidth();
	 * 
	 * if ( this.getPrefWidth() == 0 ) return Math.max(textSize, computedSize); else
	 * return computedSize; }
	 * 
	 * @Override protected double computePrefHeight() { float textSize =
	 * getTextBounds()[3]-getTextBounds()[1]; textSize += this.padding.getHeight();
	 * 
	 * double computedSize = super.computePrefHeight();
	 * 
	 * if ( this.getPrefHeight() == 0 ) return Math.max(textSize, computedSize);
	 * else return computedSize; }
	 */
	...
	/**
	 * Set the background color of this node. <br>
	...
	public void render(Context context) {
		...
		for (int i = 0; i < backgrounds.size(); i++) {
			backgrounds.get(i).render(context, getX(), getY(), getWidth() - border.getWidth(),
					getHeight() - border.getHeight(), new float[4]);
		}
