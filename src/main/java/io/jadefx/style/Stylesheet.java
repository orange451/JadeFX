...
public class Stylesheet {
	...
	public void applyStyling(Node node) {
		...
		// JUST FOR NORMAL SELECTOR STYLING FIRST
		{
			...
			List<String> classList = node.getClassList();
	...
	private void parseContent(List<StyleSelector> selectors, String content) {
		...
		Map<String, StyleVarArgs> data = new HashMap<>();
		...
		for (int i = 0; i < selectors.size(); i++) {
			...
			data.entrySet().forEach(entry -> {
				...
				if ( op != null ) {
					...
					operation.styleKey = entry.getKey();
	...
	private StyleFunctionValue parseFunction(String functionName, String text) {
		StyleFunction sFunc = StyleFunctionDefinitions.operations.get(functionName);
		if ( sFunc == null )
			sFunc = StyleFunctionDefinitions.NOFUNC;
		...
		StyleFunctionValue ret = new StyleFunctionValue();
		ret.args.add(new StyleParams());
		...
		for (int i = 0; i < text.length(); i++) {
			...
			if ( c == '(' ) {
				StyleFunctionValue func = parseFunction(currentWord.trim(), text.substring(i+1));
				...
				if ( func != null ) {
					i += func.len;
					readLen += func.len;
					ret.args.get(0).add(func.function);
				}
				continue;
			}
			...
			if ( c == ',' || c == ')' ) {
				if ( current.length() > 0 ) {
					Object value = ParseUtil.parseVal(current.trim());
					if ( value != null )
						ret.args.get(0).add(value);
				}
				current = "";
				if ( c == ')') {
					success = true;
	...
	private StyleVarArgs parseArgs(String content) {
		...
		for (int i = 0; i < content.length(); i++) {
			...
			// Parse function
			if ( c == '(' ) {
				StyleFunctionValue func = parseFunction(currentWord.trim(), content.substring(i+1));
				if ( func != null ) {
					i += func.len;
					temp.add(func);
				}
	...
	public static void findAndApplyStyle(List<Stylesheet> sheets, Node applyNode, Node parentNode, StyleOperation... operations) {
		...
		for (int i = 0; i<sheets.size(); i++) {
			...
			// Apply styling for the class
			List<String> classList = parentNode.getClassList();
